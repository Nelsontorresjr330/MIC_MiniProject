...
class undo(PluginBase):
  def main(self):
    ...
    operations = {}
    dests = []
    for i, childPath in enumerate(core.get_children_paths(active_node)):
      ...
      if (core.is_instance_of(child, META["Operation"])): 
        ...
        operations[i] = [op,src,dest,op_node]
    ...
    if len(ops_in_order) > 0:
      ...
      logger.info('No operations to delete')
