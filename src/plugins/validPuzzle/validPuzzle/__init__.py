...
The validPuzzle-class is imported from both run_plugin.py and run_debug.py
"""
import sys
import logging
...
# Setup a logger
logger = logging.getLogger('validPuzzle')
...
class validPuzzle(PluginBase):
  def main(self):
    ...
    if len(numbers) != 6 or len(operations) > 5:
      valid = False
    ...
    if valid or not contains_pos:
      logger.info("Puzzle is valid. :-)")
    else: 
      logger.info("Puzzle is not valid.:-(")
